...
// aqui além de reduce será necessário utilizar também a função map . Dica: Você pode acessar o index 
...
function studentAverage(students, grades) {
  return grades.map((element, index) => {
    return {name: students[index], average: element.reduce((acc, current) => acc + current) / element.length};
  });
}
